                       ...
                       size_t *address_length_p)
{
    ...
    msg_hdr->type      = type;
    msg_hdr->score     = ucs_balancer_get_score(ep);
...
static UCS_F_NOINLINE ucp_ep_h
ucp_wireup_prepare_ep(ucp_worker_h worker,
...
static UCS_F_NOINLINE void
ucp_wireup_process_demo_resp(ucp_worker_h worker, ucp_ep_h ep,
                                   const ucp_wireup_msg_t *msg,
                                   const ucp_unpacked_address_t *remote_address)
{
    unsigned addr_indices[UCP_MAX_LANES];
                                           ...
                                           size_t length, unsigned flags)
{
    ...
    if (msg->type == UCP_WIREUP_MSG_ACK) {
        ...
        ucp_wireup_process_demo_resp(worker, ep, msg, &remote_address);
                                  ...
                                  ucs_queue_head_t *replay_pending_queue)
{
    ...
    *connect_lane_bitmap = UCS_MASK(new_key->num_lanes);
    ...
    if (ep->cfg_index == UCP_WORKER_CFG_INDEX_NULL) {
        ...
        ucp_wireup_get_dst_rsc_indices(ep, new_key, remote_address, addr_indices, new_key->dst_rsc_idx);
    ...
    ucp_wireup_get_dst_rsc_indices(ep, new_key, remote_address, addr_indices, dst_rsc_idx);
    old_key = &ucp_ep_config(ep)->key;
    ucp_ep_config_lanes_intersect(old_key, old_key->dst_rsc_idx, new_key,
                                  dst_rsc_idx, reuse_lane_map);
...
ucs_status_t ucp_wireup_send_request(ucp_ep_h ep)
{
    ...
    ucp_ep_update_flags(ep, UCP_EP_FLAG_CONNECT_REQ_QUEUED, UCP_EP_FLAG_LOCAL_CONNECTED | UCP_EP_FLAG_REMOTE_CONNECTED);
