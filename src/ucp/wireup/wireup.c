...
static void ucp_wireup_extract_pending(ucp_ep_h ep,
                                       ucp_lane_index_t lane,
    ...
    *connect_lane_bitmap = UCS_MASK(new_key->num_lanes);
    ...
    if (should_reuse) {
        ...
        status = ucp_wireup_replace_ordered_lane(ep, new_key, new_uct_eps,
                                                 reuse_lane_map, replay_pending_queue);
        if (status != UCS_OK) {
            return status;
        }
    }
    ...
    for (lane = 0; lane < ucp_ep_num_lanes(ep); ++lane) {
        ...
        if (reuse_lane == UCP_NULL_LANE) {
            if (uct_ep != NULL) {
                ucs_assert(lane != ucp_ep_get_cm_lane(ep));
                ucp_wireup_handle_discarded_uct_ep(ep, lane, replay_pending_queue);
                ucp_ep_set_lane(ep, lane, NULL);
            }
        } else if (uct_ep != NULL) {
            if (!ucp_wireup_ep_test(uct_ep) ||
    ...
    /* Start non-blocking flush for outstanding messages */
