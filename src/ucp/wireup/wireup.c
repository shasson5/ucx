                             ...
                             const ucp_lane_index_t *lanes2remote)
{
    ...
    for (ep_index = 0; ep_index < address->num_ep_addrs; ++ep_index) {
        if (!ucp_ep_lane_reused(address->ep_addrs[ep_index].lane,
                                lanes2remote)) {
                           ...
                           ucp_lane_index_t *lanes2remote)
{
    ...
    for (lane = 0; lane < num_lanes; ++lane) {
        ...
        remote_lane   = ucp_ep_get_remote_lane(ucp_ep_get_lane(ep, lane),
                                               address);

        if (remote_lane == UCP_NULL_LANE) {
            ep_addr_index = ucp_wireup_get_next_ep_index(num_lanes, address,
                                                         lane, address_index,
                                                         lanes2remote);
