...
static unsigned ucp_worker_progress_usage_tracker(void *arg)
{
    ...
    if ((worker->usage_tracker.iter_count++ % UCP_WORKER_SLOW_CB_ITER_SKIP) != 0) {
        return 0;
    }
...
void ucp_worker_track_ep_usage(ucp_worker_h worker, ucp_ep_h ep)
{
    ...
    if ((worker->usage_tracker.samples_count % 1000000) == 0) {
        ...
        uct_worker_progress_register_safe(worker->uct, ucp_worker_progress_usage_tracker,
                                          worker, 0, &worker->usage_tracker.cb_id);
    }
}
                                                    ...
                                                    const ucs_usage_tracker_config_t *config)
{
    ...
    if (!worker->context->config.ext.enable_usage_tracker) {
        return UCS_OK;
    }
