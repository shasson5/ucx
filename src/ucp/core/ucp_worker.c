...
static void ucp_worker_flush_deferred_ep_complete(uct_completion_t *self)
{
    ...
    if ((self->count == 0) && (deferred->flush.curr == NULL)) {
        req->status = UCS_OK;
    }
...
static ucs_status_t ucp_worker_flush_deferred_ep_progress(uct_pending_req_t *self)
{
    ...
    ucs_array_for_each(uct_ep_p, &deferred->flush.uct_eps) {
        if ((deferred->flush.curr != NULL) &&
            ((*uct_ep_p) != deferred->flush.curr)) {
            continue;
        }

        status = uct_ep_flush(*uct_ep_p, UCT_FLUSH_FLAG_LOCAL, &req->send.state.uct_comp);
        if (status == UCS_INPROGRESS) {
            ++req->send.state.uct_comp.count;
        }

        else if (status == UCS_ERR_NO_RESOURCE) {
            deferred->flush.curr = *uct_ep_p;
            return UCS_ERR_NO_RESOURCE;
        }
    }

    deferred->flush.curr = NULL;
    return UCS_OK;
}
...
ucs_status_t ucp_worker_flush_deferred_ep(ucp_ep_h ucp_ep)
{
    ...
    if (ucs_unlikely(req == NULL)) {
        ucs_error("unable to allocate request for deferred EP %p on UCP "
                  "worker %p", ucp_ep, ucp_ep->worker);
        return UCS_ERR_NO_MEMORY;
    }
